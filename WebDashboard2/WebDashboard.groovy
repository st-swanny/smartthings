...
\margl1440\margr1440\vieww24440\viewh14600\viewkind0
...
\
preferences \{\
	page( name:"mainPage", title:"Set Up Page", nextPage:"reorderPage", uninstall:true, install:false ) \{\
...
\
        section("Dashboard Preferences...") \{\
            ...
            input "clockMode", title: "Clock Mode", "enum", multiple: false, required: true, defaultValue: "24 hour", metadata: [values: ["24 hour", "12 hour"]]\
        ...
        \}\
    \}\
    page(name: "reorderPage")\
\}\
\
def getDeviceIndex(def id) \{\
	\
    if (settings["deviceorder$\{id\}"]) \{\
    	// we have an id\
        return settings["deviceorder$\{id\}"]\
    \} else \{\
    	return 1000\
    \}\
\}\
\
def getOrderedDeviceMap() \{\
	def deviceMap = [:]\
\
	deviceMap["refresh"] = [type:"refresh", device:null, index: getDeviceIndex("refresh")]\
    if (showMode == "Yes") \{ deviceMap["mode"] = [type:"mode", device:null, index: getDeviceIndex("mode")] \}\
    if (showClock == "Yes") \{ deviceMap["clock"] = [type:"clock", device:null, index: getDeviceIndex("clock")] \}\
\
    switches.each \{ deviceMap[it.id] = [type:"switch", device:it, index: getDeviceIndex(it.id)] \}\
    dimmers.each \{ deviceMap[it.id] = [type:"dimmer", device:it, index: getDeviceIndex(it.id)] \}\
    locks.each \{ deviceMap[it.id] = [type:"lock", device:it, index: getDeviceIndex(it.id)] \}\
    contacts.each \{ deviceMap[it.id] = [type:"contact", device:it, index: getDeviceIndex(it.id)] \}\
    presence.each \{ deviceMap[it.id] = [type:"presence", device:it, index: getDeviceIndex(it.id)] \}\
    temperature.each \{ deviceMap[it.id] = [type:"temperature", device:it, index: getDeviceIndex(it.id)] \}\
    motionSensors.each \{ deviceMap[it.id] = [type:"motion", device:it, index: getDeviceIndex(it.id)] \}\
        \
    // don't have an incoming sort order, do it alphabetically\
	if (!state.installedOnce) \{\
        def typeSortOrder = ["refresh", "mode", "clock", "lock", "switch", "dimmer", "contact", "presence", "temperature", "motion"]\
\
    	deviceMap = deviceMap.sort \{ a, b -> \
        		def aType = typeSortOrder.findIndexOf\{ it == a.value.type \}\
                def bType = typeSortOrder.findIndexOf\{ it == b.value.type \}\
        		if (aType < bType) \{\
                    return -1\
                \} else if (aType > bType) \{\
                    return 1\
                \} else \{\
                	// now check string as same type\
                	return getDisplayNameFromDeviceMap(a.value) <=> getDisplayNameFromDeviceMap(b.value)\
                \}\
            \}\
    \
    \} else \{\
    	deviceMap = deviceMap.sort \{ a, b -> a.value.index.toFloat() <=> b.value.index.toFloat() \}\
    \}\
    \
	return deviceMap\
\}\
\
def getDisplayNameFromDeviceMap(def value) \{\
	if (value.device) \{\
    	return value.device.displayName\
    \} else if (value.type) \{\
    	return value.type\
    \} else \{\
    	return ""\
...
\
def reorderPage() \{\
	def deviceMap = getOrderedDeviceMap()    \
    \
    dynamicPage(name: "reorderPage", title: "Reorder Devices", uninstall:true, install:true) \{\
        section \{\
        	paragraph "Set the order of the devices in your dashboard. Decimals can be used for easier movement. For example, use 1.1 to get between 1 and 2."\
        	def deviceIndex = 1\
            deviceMap.each \{ key, value ->\
            	def title = "$\{getDisplayNameFromDeviceMap(value)\} - $\{value.type\}"\
                if (value.device == null) \{\
                	title = "$\{value.type\}"\
                \}\
            	input "deviceorder$\{key\}", "decimal", title:title, required: true, defaultValue:"$\{deviceIndex\}"\
...
\
def list() \{\
	render contentType: "application/javascript", data: "$\{params.callback\}($\{dataList().encodeAsJSON()\}\})"\
\}\
\
// data in a list view for easier debugging by type\
def dataList() \{\
    [\
        ...
        motion: motionSensors?.collect\{[type: "motion", id: it.id, name: it.displayName, status: it.currentValue('motion')]\}?.sort\{it.name\},\
    ]\
\}\
\
// data in the ordered view\
def data() \{\
\
	def deviceMap = getOrderedDeviceMap()\
    \
    return deviceMap.collect \{ key, it ->\
    	def itMap = [type: it.type, id: key, name: getDisplayNameFromDeviceMap(it)]\
        switch (it.type) \{\
        	case "lock": itMap.status = it.device.currentValue('lock') == "locked" ? "locked" : "unlocked"; break\
            case "switch": itMap.status = it.device.currentValue('switch'); break\
            case "dimmer": itMap.status = it.device.currentValue('switch'); itMap.level = it.device.currentValue('level'); break\
            case "contact": itMap.status = it.device.currentValue('contact'); break\
            case "presence": itMap.status = it.device.currentValue('presence'); break\
            case "temperature": itMap.status = it.device.currentValue('temperature'); break\
            case "motion": itMap.status = it.device.currentValue('motion'); break\
        \}\
        \
        return itMap\
    \}\
...
def body() \{\
	def devices = ""\
    data().each\{devices = devices + renderDevice(it)\}\
    \
	"""\
...
function startTime() \{\
    ...
    s = checkTime(s);\
    var post = "";\
    $\{clockMode == "24 hour" ? "" : """h=h%12;post = " am";if (h!=oldh) \{post = " pm"\};"""\}\
    document.getElementById('clock').innerHTML = h+":"+m+post;\
...
<div id="device_$device.id" class="st-tile $device.type">\
	<div class="st-tile-content">\
        ...
        <div class="st-icon">\
        	<i class="r45 fa $\{device.status == "active" ? "fa-arrows" : "fa-circle-o"\}"></i>\
        </div>\
	</div>\
</div>\
"""\
...
def renderDevice(device) \{\
    if (!device) return ""\
    if (device.type == "clock") return renderClock()\
    if (device.type == "mode") return renderMode()\
    if (device.type == "refresh") return renderRefresh()\
    \
